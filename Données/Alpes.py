...
CSV_PATH = "alpes/new_ids.csv"
OUTPUT_TTL = "alpes/frontlet_import_output.ttl"
MAPPING_PATH = "alpes/mappingAlpesThesaurusCauseMort.csv"  # Fichier de mapping CSV (optionnel)
...
for idx, row in df.iterrows():
    ...
    # DEATH EVENT
    event_uri = DATA["alpes_Death_gen_%d" % (idx+1)]
    ...
    date_mort_val = row.get("Date_mort", "") or row.get("date_mort", "")
    ...
    if cause_deces_val and not is_missing(cause_deces_val):
        ...
        if uri:
            # Triple: Death hasDeathCause ThesaurusURI
    ...
    if source_val and not is_missing(source_val):
        ...
        if source_str.lower().startswith('http'):
            source_uri = DATA["alpes_Source_%d" % (idx+1)]
